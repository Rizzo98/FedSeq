...
class Attacker(FedAvgClient):
    def __init__(self, client_id: int, dataloader: Optional[DataLoader], G_class, D_class, outputFolder, saveImages, saveLoss,num_classes=None, device="cpu", dp=None):
...
class FedSeqGANattack(FedSeq):
    def __init__(self, model_info, params, device: str, dataset, output_suffix: str, savedir: str, writer=None, wandbConf=None):
        ...
        sc, c, pos = self.__injectAttacker(G_class, D_class, self.__outputFolder, self.__saveImages, self.__saveLoss)
    ...
    def __injectAttacker(self, G_class, D_class, outputFolder, saveImages, saveLoss):
        ...
        self.attacker = Attacker(client.client_id,client.dataloader,G_class,D_class,outputFolder,saveImages,saveLoss,client.num_classes,client.device,client.dp)
